...
def set_chart_xticks(xvalues: list[str | int | float | datetime], ax: Axes, percentage: bool = False) -> Axes:
    if len(xvalues) > 0:
        ...
        if not any(not isinstance(x, (int, float)) for x in xvalues):
            ...
            rotation = 45 if len(xvalues) > 3 else 0
...
def plot_multibar_chart(
    ...
    # Rotate xticks
    ax.tick_params(axis="x", labelrotation=45, labelsize="xx-small")
...
def encode_cyclic_variables(df: DataFrame, vars: list[str]) -> None:
    for v in vars:
...
) -> tuple[DataFrame, OneHotEncoder]:
    other_vars: list[str] = [c for c in df.columns if c not in vars_to_dummify]

    for col in vars_to_dummify:
        if col not in df.columns:
            df[col] = "UNKNOWN"

    if encoder is None:
        enc = OneHotEncoder(handle_unknown="ignore", sparse_output=False, dtype="float64", drop="if_binary")
        trans: ndarray = enc.fit_transform(df[vars_to_dummify])
    else:
        enc = encoder
        trans = enc.transform(df[vars_to_dummify])

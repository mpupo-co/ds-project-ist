...
# Recreate the same train-test split for consistency
trnX, tstX, trnY, tstY = train_test_split(X, y, train_size=0.7, stratify=y, random_state=42)

# List of model study functions
model_studies = {
    "Naive Bayes": naive_Bayes_study,
    "Logistic Regression": logistic_regression_study,
    "KNN": knn_study,
    "Decision Tree": trees_study,
    "MLP": mlp_study
}

results = []

# Loop through each model type and collect metrics
for name, study_func in model_studies.items():
    print(f"\n{'='*15} {name} Study {'='*15}")
    if name == "Logistic Regression":
        best_model, params = study_func(trnX, trnY, tstX, tstY, nr_max_iterations=1000, lag=250, metric='accuracy')
    elif name == "KNN":
        best_model, params = study_func(trnX, trnY, tstX, tstY, k_max=20, metric='accuracy')
    elif name == "Decision Tree":
        best_model, params = study_func(trnX, trnY, tstX, tstY, d_max=20, metric='accuracy')
    elif name == "MLP":
        best_model, params = study_func(trnX, trnY, tstX, tstY, nr_max_iterations=1000, lag=250, metric='accuracy')
    else:  # Naive Bayes
        best_model, params = study_func(trnX, trnY, tstX, tstY, metric='accuracy')
    
    # Predictions
    prdY = best_model.predict(tstX)
    
    # Compute metrics
    acc = accuracy_score(tstY, prdY)
    prec = precision_score(tstY, prdY, average='weighted', zero_division=0)
    rec = recall_score(tstY, prdY, average='weighted', zero_division=0)
...
# Save to CSV
results_df.to_csv(f'results/{file_tag}_best_models_comparison.csv', index=False)

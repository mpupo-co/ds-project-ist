...
if [] != numeric:
    ...
    data[numeric].boxplot(rot=45, ax=ax, whis=20)
    tight_layout()
    savefig(f"images/{file_tag}_global_boxplot_20.png")
else:
...
if [] != numeric:
    ...
    for n in range(len(numeric)):
        ...
        axs[i, j].boxplot(data[numeric[n]].dropna().values, whis=20)
        i, j = (i + 1, 0) if (n + 1) % cols == 0 else (i, j + 1)
    savefig(f"images/{file_tag}_single_boxplots_20.png")
else:
    print("There are no numeric variables.")
...
if [] != numeric:
    outliers = count_outliers(data, numeric, nrstdev=4, iqrfactor=20)
    ...
    if filtered_numeric:
        ...
        savefig(f"images/{file_tag}_outliers_20.png")
    else:

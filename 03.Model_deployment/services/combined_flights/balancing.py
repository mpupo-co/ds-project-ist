...
def data_balancing(train_df: DataFrame, target: str, strategy: str = "under", random_state: int = 42) -> DataFrame:
    print(f"=== Applying data balancing strategy: {strategy} ===")
    if strategy == "under":
        df_balanced = under_balancing(train_df, target)
    elif strategy == "over":
        df_balanced = over_balancing(train_df, target)
    elif strategy == "smote":
        df_balanced = smote_balancing(train_df, target, random_state=random_state)
    else:

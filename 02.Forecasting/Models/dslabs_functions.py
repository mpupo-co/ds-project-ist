...
def plot_bar_chart(
    ...
    # comment the 2 lines above to avoid bar labels
...
def plot_multibar_chart(
    ...
    bar_width: float = 0.8 / len(bar_labels) if len(bar_labels) > 0 else 0.8
...
def plot_forecasting_eval(
    ...
    def _replace_bar_labels(ax, fmt: str = "{:.2f}", fontsize: int = 9):
        # Remove existing labels
        for txt in ax.texts:
            txt.remove()

        # Add new labels with smaller font
    ...
    _replace_bar_labels(axs[0], fontsize=5.5)
    ...
    _replace_bar_labels(axs[1], fontsize=5.5)

...
from services.encoding import data_encoding
from services.outliers import get_variable_types, process_outliers
from services.balancing import data_balancing
from services.feature import feature_selection
import time
...
class DataPipeline:
    ...
    def fit(self, df: DataFrame) -> DataFrame:
        ...
        df_prepared.to_csv("data/processed_data.csv", index=False)
        self.fitted = True
        return df_prepared
    ...
    def _fit_internal(self, df: DataFrame) -> DataFrame:
        ...
        if self.balancing:
            balanced = data_balancing(
                concat([train_df, test_df]),
                target=self.target,
                strategy=self.balancing,
            )
            train_df, test_df = split_train_test_data(balanced)
    ...
    def _init_scaler(self, train_df: DataFrame) -> None:
        feature_cols = [c for c in train_df.columns if c != self.target]
